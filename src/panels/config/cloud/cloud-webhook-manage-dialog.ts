  ...
  protected render() {
    ...
    return html`
      ...
      <paper-dialog with-backdrop>
        <h2>Webhook for ${entry.name}</h2>
        <div>
          <p>The webhook is available at the following url:</p>
          <paper-input
            label="${inputLabel}"
            value="${entry.cloud_url}"
            @click="${this._copyClipboard}"
            @blur="${this._restoreLabel}"
          ></paper-input>
          <p>If you no longer want to use this webhook, you can
          <button
            class="link"
        ...
        <div class="paper-dialog-buttons">
          <a href="#" target="_blank"><paper-button
          >VIEW DOCUMENTATION</paper-button></a>
          <paper-button
